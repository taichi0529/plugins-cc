...
ビルド・テストランナーはない。フックは shell + `jq`、skill は Markdown 手順書。確定済み設計判断(D1〜D21)とテスト計画は `docs/handoff.md` にある。
