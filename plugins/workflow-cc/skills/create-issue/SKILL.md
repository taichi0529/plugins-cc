...
| DoD | ① `.claude/workflow-cc.json` のルート `dodFiles` + 起票対象の Target scope にマッチする `scopes[]` の `dodFiles` (パス配列・union) → ② 慣例パス `.claude/dod/*.md` (全ファイル) → ③ どちらも無ければ **DoD セクションを省略して起票** |
| ベースブランチ | 設定ファイル (同上) の `baseBranch` → 無ければ `gh repo view --json defaultBranchRef -q .defaultBranchRef.name` (Task のブランチ/PR 説明に使う) |

設定ファイルのスキーマ・解決規則の正典は implement-issue SKILL.md の「リポジトリ設定の解決」。create-issue は変更 diff を持たないため **repo-wide フィールドのみ**使い、scope 別の `dodFiles` は起票時に Target scope が宣言される場合だけ union する。
...
## Definition of Done
<!-- 解決済み DoD (.claude/workflow-cc.json の dodFiles / .claude/dod/*.md)。無ければこのセクションごと省略 -->
