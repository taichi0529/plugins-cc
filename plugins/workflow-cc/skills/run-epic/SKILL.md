...
review 0 件はタスクの途中です。親への return は、Phase B で次の 2 点を実 shell で確認した後にしてください (親はこの結果を再検証し、未確認の success は差し戻されます):
...
2. <SKILL_PATH> を Read して、その「リポジトリ設定の解決」「レビュー系実行体のモデル解決」「アルゴリズム」「イデンポテント実行手順」セクションに従って Issue #<N> を実装する。
   ...
   - implement-issue が "success" を返したら Phase B に進む
...
レビュー (特に Skill 経路で現セッションから起動した /security-review) が return した直後は、そのレポートを最終応答にせず、次のツール呼び出し (PR コメント投稿 → Phase B の実 shell 確認) に進んでください。過去にここでレポートを最終応答にして停止し、Phase B が抜けた例があります。
```
