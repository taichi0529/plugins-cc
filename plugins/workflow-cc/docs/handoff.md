...
作成: 2026-07-13 / 作成元セッション: 非公開リポジトリでの設計議論
...
プロジェクト A / B で運用している「複数 issue 一括 + ループ実行」ワークフローを、
...
- 元になる skill ファイルは プロジェクト A のリポジトリ内にある (§12)。同一マシンなら**読み取り専用で参照可**。
  A/B リポジトリのファイルは編集しないこと。
...
- 課題 2: skills を A → B に手作業移植している (docs/hygienist.md が移植指示書)。
...
| D13 (v0.5.0) | リポジトリ設定は `.claude/workflow-cc.json`(単一ファイル + `scopes[]` path-scope)のみ。旧 `.claude/workflow.json` は**廃止・読まない**(リネームで移行、検出時は案内)。repo-wide (baseBranch/trustCI) は起動時、scope 依存 (gates/reviewers/dodFiles) は**ゲート実行直前に変更ファイル集合から**二段階解決 | モノレポで app / backend 担当の gates が単一フラットファイルを取り合う衝突の解消。プラグイン名前空間のファイル名で他ツールとの衝突も回避。単一ファイルはディレクトリ分割案より LLM 実行の失敗面が狭い (Codex/Grok 両レビュー一致)。フォールバック分岐を持たないことで解決規則を単純化 (ユーザー判断で切り捨て) |
| D14 (v0.5.0) | run-epic は既定直列のまま、`parallel=N` opt-in で wave 並列。独立判定は**明示宣言のみ** (`depends on:` + `Target scope`、LLM の内容予測は不使用)。依存はコード依存として ready-set 方式 (依存先の merge を待ち、同一実行内で wave 間にコードを流さない)。並列度は個人・マシン都合のため設定ファイルに入れず引数で指定 | 独立 Sub-issue の待ち時間削減。auto-merge しない以上 wave 間で成果は流れないという事実に設計を合わせる (stacked PR は複雑すぎるため不採用)。宣言なし issue は保守的直列で後方互換 |
| D15 (v0.6.0) | run-epic の `model=<名>` 引数で**子エージェントの spawn にだけ**モデルを指定可 (省略時はセッションモデル継承)。値の allowlist は SKILL に持たずパススルー、spawn 拒否時はフォールバックせず停止。設定ファイルには入れない | モデル選択は個人のコスト許容度の問題 (D14 の並列度と同じ理由)。allowlist を持つと harness 側のモデル追加のたびに SKILL 更新が必要になる。無断フォールバックは「指定したモデルで動いた」という前提を裏切るため禁止 |
| D16 (v0.7.0) | implement-issue は**最初の PR 作成前に 1 回** code-simplifier plugin の `/simplify` を実行してから PR を作る (Step 3.5)。simplify 後は解決済みゲートを再実行して pass 必須。PR 作成後の修正ループでは再実行しない。skill が環境に無い / docs-only の場合は skip して最終報告に明記 (fail-open) | レビュー前に機械的な整理を済ませ、レビュー指摘を本質的な問題に集中させる。PR 後に再実行するとレビュー対応 diff が膨らみ採否検証が難しくなる。fail-open は D10 と同じ思想 (環境依存の追加要素で本体のループを止めない) |
...
│ ├── .claude/workflow-cc.json  # reviewers 等 (§9・任意)    │
...
元ファイル: プロジェクト A のリポジトリの `.claude/skills/{implement-issue,run-epic,create-issue}/SKILL.md` (§12)。
**コピーして持ってくるが、以下の改修を必ず行う**:

### 7.1 全 skill 共通 (脱ハードコード)

- repo slug (`<org>/<repo>`) → `gh repo view --json nameWithOwner` か `git remote get-url origin` から導出。
- ベースブランチ (`dev`) → `gh repo view --json defaultBranchRef` を既定とし、`.claude/workflow-cc.json` の
  `baseBranch` があればそちらを優先 (A/B は default が dev でない可能性があるため)。
- ローカルゲート (`npm run type-check` / `npm run lint`) → package.json の scripts から存在するものを
  実行 (`type-check`, `lint`, `test` を探す)。無ければ `.claude/workflow-cc.json` の `gates` 配列。
  どちらも無ければ「ゲート無し」と最終報告に明記して続行。
- 「CI は信頼しない」ポリシー → プロジェクト A 固有の事情なので **workflow-cc.json の `trustCI: false` (既定 true)** に
  設定化。false のときのみ現行の「gh pr checks を見ない」挙動。
- **掃除対象 (プロジェクト A 版からコピーする際に削除する古い記述)**: `patches/` (patch-package) への言及、
  特定 SDK / IDE バージョン不整合の Gotcha、`Co-Authored-By: Claude Opus 4.7` 固定行 (現行モデル名に
  するか設定化)、プロジェクト固有の死にコード Gotcha。これらはリポジトリ側 CLAUDE.md の
...
- テンプレート骨格 (feature/bug/refactor/chore/epic/task、AC の Given/When/Then、Sub-issues API 手順)
  は汎用なので plugin に同梱。
- **DoD はリポジトリ側データ**: `.claude/workflow-cc.json` の `dodFiles` (パス配列) か、慣例パス
  (`.claude/dod/*.md`) を読む。無ければ DoD セクションを省略して起票。
  プロジェクト A の `dod/mobile.md` 相当はモバイルリポジトリ側にのみ置く。
...
2. リポジトリ設定: `.claude/workflow-cc.json` の `reviewers` 配列 (例: `["project", "codex"]`)
3. 既定: `["project", "adversarial", "grok"]` (project = review skill、無ければ公式 `/code-review`。adversarial = D19 の red-team。grok = D21 の別エンジン常設)
...
- **2 回目以降のラウンドは差分照合モード** (D18): 前ラウンドで指摘を出したレビュアーのみ、
  前回指摘リスト + 修正 diff を渡して解消判定させる (全量再実行しない)。
...
## 9. リポジトリ側契約: `.claude/workflow-cc.json` (すべて任意)

> v0.5.0 (D13) で `.claude/workflow-cc.json` (path-scope 対応) のみとなり、旧 `.claude/workflow.json` は
> **廃止**された (リネームで移行。`scopes` 無しのフラット形式はそのまま有効。検出時は skill が案内を出す)。
> スキーマ・キー許可リスト・prefix マッチ規則・二段階解決は
> **`skills/implement-issue/SKILL.md` の「リポジトリ設定の解決」が正典**。

フラット形式の例 (`scopes` 無し・最小構成):
...
壊れても困らないこと。本番リポジトリ (プロジェクト A / B) では**テストしない**。
...
3. 検証項目:
   - [ ] repo slug / ベースブランチ / ゲートが自動導出される (workflow-cc.json 無しで動く)
...
### Phase 5: wave 並列 + path-scope 設定 (v0.5.0 / D13・D14)

1. Sub-issue 3〜4 本 (うち 2 本は異なる `Target scope`、1 本は `depends on:` 付き) の EPIC で検証。
2. 検証項目:
   - [ ] `parallel` 未指定で直列・トリアージ無し (処理順・停止動作・検証は 0.4.x と同一。ブランチ親割当と検証 NG PR 再分類のみ変更) — 回帰
   - [ ] `parallel=2` + 明示 depends/scope で独立子だけが同一メッセージ並列 spawn される
   - [ ] 宣言なし子は保守的直列 (エラーにならない)
   - [ ] `depends on:` の依存先が未 merge の子は「merge 待ち」で今回未処理になる
   - [ ] 閉路宣言で実行前に停止する
   - [ ] main checkout モード + `parallel=2` は警告して 1 に落ちる
   - [ ] バッチ内 1 子 failure で in-flight は検証・記録され、新バッチが開始されない
   - [ ] 検証 NG のまま停止 → 再実行で該当子が作業リストに戻る (merge 待ち扱いで skip されない)
   - [ ] `workflow-cc.json` あり / 無し (自動導出) の解決が正しい。旧 `workflow.json` のみ存在する場合は**読まれず**、リネーム案内が最終報告に出る
   - [ ] `scopes[]` 内の `baseBranch`/`trustCI` 記載で停止する (キー許可リスト)
   - [ ] prefix 境界: `apps/web` が `apps/web-old` にマッチしない
   - [ ] staged / unstaged / untracked の変更が scope 解決の変更ファイル集合に含まれる

### Phase 6: model= 引数 (v0.6.0 / D15)

1. Phase 5 と同じ小さな EPIC で検証。
2. 検証項目:
   - [ ] `model` 未指定で子の Agent 呼び出しに model パラメタが付与されない (= セッションモデル継承)
...
### Phase 8: grok 既定化 + レビュー並列化 (v0.12.0 / D21)

1. Phase 2 と同じ小さな issue 1 件で検証。
2. 検証項目:
   - [ ] `review=` 未指定・設定ファイル無しで project / adversarial / grok の 3 体が**同一メッセージ内の Agent 呼び出し**で起動される (1 体ずつ待たない)
   - [ ] grok-cc plugin が無い / Grok CLI 未認証の環境で停止せず、最終報告に `grok: 利用不可` が出る
   - [ ] `review=project,adversarial` で grok が起動されない。`workflow-cc.json` の `reviewers` 指定時も既定の grok は足されない
   - [ ] トリアージが全レビュアーの結果が揃った後に 1 回だけ行われる (先着の指摘で修正を始めない)
   - [ ] 差分照合ラウンドで再実行対象が 2 体以上のとき並列起動される
...
## 12. 参照元ファイル (プロジェクト A のリポジトリ・読み取り専用)

| パス | 用途 |
|---|---|
| `<プロジェクト A のリポジトリ>/.claude/skills/implement-issue/SKILL.md` | ループアルゴリズムの原本 (§7.1 の掃除対象を除去して汎用化) |
| `<プロジェクト A のリポジトリ>/.claude/skills/run-epic/SKILL.md` | EPIC 分解・worktree 並列の原本 |
| `<プロジェクト A のリポジトリ>/.claude/skills/create-issue/SKILL.md` (+ `templates/`, `dod/`) | 起票フローの原本。dod/ はリポジトリ側データ化 |
| `<プロジェクト A のリポジトリ>/.claude/skills/code-review-project/SKILL.md` | plugin には入れない。フォールバック仕様 (D8) の参照用 |
| `<プロジェクト A のリポジトリ>/docs/hygienist.md` | 「機械実行可能な引き継ぎ資料」の先行例 (書式の参考) |
