...
function probeGrokAuth(cwd) {
  ...
  return {
    ...
    // `grok models` reports the auth snapshot from *before* it lazily refreshes
    // an expired-but-renewable OIDC token, so a token that is about to be
    // refreshed shows up as "not authenticated" on the first probe even though
    // that same process refreshes it and writes the new token to disk before
    // exiting. A clean (non-error) miss is therefore worth one re-probe; a
    // spawn failure or timeout is not.
...
export async function getGrokAuthStatus(cwd) {
  ...
  // The first probe can echo a stale "not authenticated" banner while grok
  // refreshes the token behind it; the refreshed token lands on disk before
  // that process exits, so a single re-probe reads it and reports the true
  // state. A genuinely logged-out user just sees the same miss twice.
  let probe = probeGrokAuth(cwd);
  if (!probe.status.loggedIn && probe.transient) {
    probe = probeGrokAuth(cwd);
  }
  return probe.status;
}
