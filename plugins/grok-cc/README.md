...
| Session lifecycle hooks | Export `GROK_COMPANION_SESSION_ID` and related env; clean up jobs on session end |

Review output is structured JSON (schema-validated), then rendered as Markdown. Write-capable tasks use the Grok sandbox profile `workspace`; reviews and read-only tasks use `read-only`. Job state is stored under `GROK_COMPANION_DATA` (falls back to `CLAUDE_PLUGIN_DATA` inside plugin hooks, then to `grok-companion/` in the system temp dir) and is filtered by Claude session when possible.
...
Every slash command ultimately runs (or is built around) `scripts/grok-companion.mjs`. You can call it directly for debugging or automation (paths relative to this plugin directory):
...
|---|---|---|
| `SessionStart` | `session-lifecycle-hook.mjs` | Exports `GROK_COMPANION_SESSION_ID`, `GROK_COMPANION_DATA`, and the transcript path so jobs can be scoped to the Claude session |
...
Paths below are relative to this plugin directory.
