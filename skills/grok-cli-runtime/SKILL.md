...
Use this skill only inside the `grok-cc:grok-rescue` subagent.
...
- Do not call `setup`, `review`, `adversarial-review`, `status`, `result`, or `cancel` from `grok-cc:grok-rescue`.
...
Safety rules:
- Default to write-capable Grok work in `grok-cc:grok-rescue` unless the user explicitly asks for read-only behavior.
