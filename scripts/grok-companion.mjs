...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/grok-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model|fast>] [--effort <none|minimal|low|medium|high|xhigh|max>] [prompt]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!grokStatus.available) {
    nextSteps.push("Install Grok with `curl -fsSL https://x.ai/cli/install.sh | bash`.");
  }
...
function findLatestResumableTaskJob(jobs) {
  return (
    jobs.find(
      (job) =>
        job.jobClass === "task" &&
        job.title !== STOP_REVIEW_JOB_TITLE &&
        job.threadId &&
        job.status !== "queued" &&
        job.status !== "running"
    ) ?? null
...
async function executeReviewRun(request) {
  ...
  if (reviewName === "Review") {
    validateStandardReviewRequest(target, focusText);
  }
...
function enqueueBackgroundTask(cwd, job, request) {
  ...
  // The worker reads the job file on startup, so it must exist before spawn.
  const queuedRecord = {
    ...job,
    status: "queued",
    phase: "queued",
  ...
  // Patch only the pid afterwards; a full rewrite could clobber the worker's
  // own "running" record if it boots faster than this parent finishes.
...
async function handleCancel(argv) {
  ...
  const interrupt = await interruptGrokTurn(cwd, { threadId, turnId });
